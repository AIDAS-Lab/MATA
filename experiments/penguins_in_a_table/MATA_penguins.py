...
penguins_question_list = []
penguins_pred_list = []
penguins_answer_list = []
for index in tqdm(range(len(dataset))):
    ...
    question = penguins.get_question()
    penguins_answer = penguins.get_answer_extract()
    
    table = make_unique_columns(penguins_table) # If column names are duplicated, this function add additional numbers at columns. (ex. [Day, Month, Day, Month] => [Day 1, Month 1, Day 2, Month 2]) 
    ...
    penguins_question_list.append(question)
    penguins_pred_list.append(final_answer)
    penguins_answer_list.append(penguins_answer)

# DataFrame 생성
df = pd.DataFrame({
    "Question": penguins_question_list,
    "Prediction": penguins_pred_list,
    "Answer": penguins_answer_list
})

# CSV 저장
df.to_csv("MATA_penguins_qwen2-5_32b.csv", index=False, encoding="utf-8-sig")
